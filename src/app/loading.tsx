export default function DashboardLoading() {
  const SkeletonBar = () => (
    <div className="h-3 bg-line-card rounded-sm animate-pulse" />
  );
    ...
    <div className="max-w-lg mx-auto space-y-2.5">
      ...
      <div className="card space-y-3">
        ...
        <div className="space-y-2">
          <div className="h-6 w-3/4 bg-line-card rounded-sm animate-pulse" />
      ...
      {Array.from({ length: 3 }).map((_, i) => (
        <div key={i} className="card space-y-2">
          ...
          <div className="h-5 w-3/4 bg-line-card rounded-sm animate-pulse" />
        </div>
      ))}
    </div>
  );
